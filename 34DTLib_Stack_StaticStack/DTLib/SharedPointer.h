...
template <typename T>
class SharedPionter : public Pointer<T>
{
protected:
    int* m_ref; //计数机制成员指针

    void assign(const SharedPionter<T>& obj)
    {
...
public:
    SharedPionter(T* p = NULL) :m_ref(NULL)
    {
        if(p)
        {
    ...
    SharedPionter(const SharedPionter<T>& obj):Pointer<T>(NULL)
    {
        assign(obj);
    }


    SharedPionter<T>& operator= (const SharedPionter<T>& obj)
    {
...
template <typename T >
bool operator==(const SharedPionter<T>& l, SharedPionter<T>& r)
{
