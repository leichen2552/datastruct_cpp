...
{
protected:
    LinuxLinkQuene<GTreeNode<T>*> m_queue;//遍历需要的队列
...
public:
    ...
    SharedPointer< Tree<T> > remove(const T& value)//移除元素值
    {
        ...
        {
            remove(node, ret);//从树里面删除节点之后

            m_queue.clear();//也要把队列清除
        }
    ...
    SharedPointer< Tree<T> > remove(TreeNode<T>* node)//移除某个节点
    {
        ...
        {
            remove( dynamic_cast<GTreeNode<T>*>(node), ret);//从树里面删除节点之后
    ...
    void clear()
    {
        ...
        this->m_root = NULL;//在清空树之后

        m_queue.clear();//接着把队列里面所有元素也清空
    }
